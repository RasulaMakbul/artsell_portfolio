<x-admin.master>
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Fashion Section</h1>
        ...
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="btn-group me-2">
                ...
                <button type="button" class="btn btn-sm btn-outline-secondary" data-toggle="modal"
                    data-target="#createFashionModal">
            ...
            <tbody>
                @forelse ($fashions as $fashion)
                    <tr class="align-middle">
                        ...
                        <td>
                            <img src="{{ asset($fashion->image) }}" alt="fashion image" class="img-fluid rounded"
                                style="height: 100px; width: 100px;">
                        </td>
                        <td>{{ $fashion->title }}</td>
                        <td>
                            @if ($fashion->status == 0)
                                <a href="{{ route('fashion.active', $fashion->id) }}"
                                    class="btn btn-sm link-success">{{ __('Hidden') }}</a>
                            @else
                                <a href="{{ route('fashion.inactive', $fashion->id) }}"
                                    class="btn btn-sm link-danger">{{ __('Visible') }}</a>
                            @endif
                        </td>
                        <td>
                            {{-- <a href="{{ route('fashion.show', $fashion->id) }}" class="btn btn-sm link-info"><i
                                    class="fa-solid fa-eye fs-5"></i></a> --}}
                            <button type="button" class="btn btn-sm link-success" data-toggle="modal1"
                                data-target="#showModal{{ $fashion->id }}">
                                <i class="fa-solid fa-eye fs-5"></i></i>
                            </button>
                            <button type="button" class="btn btn-sm link-warning" data-toggle="modal2"
                                data-target="#myModal{{ $fashion->id }}">
                                <i class="fa-solid fa-pen-to-square fs-5"></i>
                            </button>
                            {{-- <a href="{{route('fashion.edit',$fashion->id)}}" class=" btn btn-sm link-warning" comment="Edit Subcategory">edit</a> --}}
    ...
    <div class="modal fade" id="createFashionModal" tabindex="-1" aria-labelledby="createFashionModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title fs-5" id="createFashionModalLabel"><span><i
                                class="fa-solid fa-plus"></i></span>{{ __(' Create New fashion') }}</h3>
    ...
    @forelse ($fashions as $item)
        <div class="modal fade" id="myModal{{ $item->id }}" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="myModalLabel{{ $item->id }}"> Update fashion
                            {{ $item->title }}
                        </h4>
                    ...
                    <div class="modal-body">
                        <form action="{{ route('fashion.update', $item->id) }}" method="POST"
