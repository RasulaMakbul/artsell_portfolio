<x-admin.master>
    <x-slot:title>
        {{ __('Architecture Section') }}
    </x-slot:title>
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Architecture Section</h1>
        ...
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="btn-group me-2">
                ...
                <button type="button" class="btn btn-sm btn-outline-secondary" data-toggle="modal"
                    data-target="#createArchitectureModal">
            ...
            <tbody>
                @forelse ($architectures as $architecture)
                    <tr class="align-middle">
                        ...
                        <td>
                            <img src="{{ asset($architecture->image) }}" alt="architecture image"
                                class="img-fluid rounded" style="height: 100px; width: 100px;">
                        </td>
                        <td>{{ $architecture->title }}</td>
                        <td>
                            @if ($architecture->status == 0)
                                <a href="{{ route('architecture.active', $architecture->id) }}"
                                    class="btn btn-sm link-success">{{ __('Hidden') }}</a>
                            @else
                                <a href="{{ route('architecture.inactive', $architecture->id) }}"
                                    class="btn btn-sm link-danger">{{ __('Visible') }}</a>
                            @endif
                        </td>
                        <td>
                            {{-- <a href="{{ route('architecture.show', $architecture->id) }}" class="btn btn-sm link-info"><i
                                    class="fa-solid fa-eye fs-5"></i></a> --}}
                            <button type="button" class="btn btn-sm link-success" data-toggle="modal1"
                                data-target="#showModal{{ $architecture->id }}">
                                <i class="fa-solid fa-eye fs-5"></i></i>
                            </button>
                            <button type="button" class="btn btn-sm link-warning" data-toggle="modal2"
                                data-target="#myModal{{ $architecture->id }}">
                                <i class="fa-solid fa-pen-to-square fs-5"></i>
                            </button>
                            {{-- <a href="{{route('architecture.edit',$architecture->id)}}" class=" btn btn-sm link-warning" comment="Edit Subcategory">edit</a> --}}
                ...
                @empty
                    <tr>
                        <td colspan="5">No architecture added!</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="modal fade" id="createArchitectureModal" tabindex="-1" aria-labelledby="createArchitectureModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title fs-5" id="createArchitectureModalLabel"><span><i
                                class="fa-solid fa-plus"></i></span>{{ __(' Create New architecture') }}</h3>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('architecture.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
        ...
        <div class="modal fade" id="myModal{{ $item->id }}" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('architecture.update', $item->id) }}" method="POST"
